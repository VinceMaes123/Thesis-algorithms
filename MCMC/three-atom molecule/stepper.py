...
def classicMCMC(x_current,solver):
    ...
    dW = math.sqrt(solver.microTimestep) * scipy.random.normal(0.0, 1.0, 3)  # brownian step at micro level
...
def indirect(x_current,eta,solver):
    ...
    gradV = UDE.totalgradPot(x_current,solver)  # vector with 3 components
    z_current = UDE.restriction(x_current)
    gradE = UDE.gradRestriction(x_current)  # vector with 3 components
    dW = math.sqrt(solver.microTimestep) * scipy.random.normal(0.0, 1.0, 3) #brownian step at micro level
