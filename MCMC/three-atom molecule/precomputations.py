...
#generate a grid over the reaction coordinate
def getMacroGrid(solver):
    z_grid = scipy.linspace(solver.zmin, solver.zmax, solver.J)
    return z_grid

#calculate A_tilde (normalization constant Z_{pm} of the indirect reconstruction density) on z_grid
...
#the effective dynamics b, sigma and the free energy A_hat are calculated on the grid.
#WORKS ONLY IF 1D REACTION COORDINATE
